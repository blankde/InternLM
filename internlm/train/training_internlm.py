...
def initialize_model():
        ...
        def hook_backward_function(module, module_input_grad, module_output_gard):  # pylint: disable=W0613
            ...
            with torch.no_grad():
                for test_tensor in module_input_grad:
                    ...
                    if not all_equal:
                        print(name, 
                               "differ: " + str((gathered_tensors[0] - gathered_tensors[1]).abs().float()), "\n",
                               flush=True)
                        assert False
    ...
    def wrapper2(name):  # pylint: disable=W0613
        def hook_backward_function2(module, module_input_grad, module_output_gard):  # pylint: disable=W0613
            from internlm.core.context.parallel_context import global_context as gpc
            # print("hook!!!",flush=True)
            with torch.no_grad():
                for test_tensor in module_output_gard:
        ...
        return hook_backward_function2
